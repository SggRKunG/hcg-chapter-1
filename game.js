...
class Game{
	...
	load(){
		const game = this;
		this.loadJSON("gems", function(data, game){
	...
	getConnectedSprites(index, row, col, connected=[]){
		...
		try{
			if (sprite.index == index && !sprite.checked){
				...
				for(let r=row-1; r<=row+1; r++){
					...
					for(let r=row-1; r<=row+1; r++){
						if (!boundaryCheck(r, 0)) continue;
						connected.concat(this.getConnectedSprites(index, r, col, connected));
					}
					for(let c=col-1; c<=col+1; c++){
						if (!boundaryCheck(0, c)) continue;
						connected.concat(this.getConnectedSprites(index, row, c, connected));
					}
				}
			}
		}catch(e){
		...
		function bomb(sprite, connected){
			if (connected >= 5){
				sprite = new Sprite({
					game: this,
					context: this.context,
